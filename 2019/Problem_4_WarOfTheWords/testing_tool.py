...
_WRONG_ANSWER_MSG = "-1"
PRINT_INTERACTION_HISTORY = True #False
...
class JudgeSingleCase(object):
  ...
  def __init__(self, io):
    ...
    self.rank = list(range(nwords))
    random.shuffle(self.rank)

    self.rank_rev = [-1 for i in range(nwords)]
...
def JudgeAllCases(test_number, io):
  ...
  for case_number in range(1, total_num_cases + 1):
